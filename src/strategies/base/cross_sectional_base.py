...
class CrossSectionalConfig:
    ...
    output_dir: str = "results/tables/cross_sectional_strategy"
...
class CrossSectionalBacktestBase:
    """
    Shared infrastructure for cross-sectional long-short factor strategies.

    Responsibilities:
    - point-in-time lifecycle universe
    - price loading/cache
    - point-in-time fundamentals
    - raw cross-sectional snapshot construction
    - generic signal filters
    - portfolio membership / weights
    - holding-period return calculation

    Strategy-specific signal logic is delegated to a BaseCrossSectionalStrategy.
    """

    def __init__(self, config: CrossSectionalConfig, strategy: BaseCrossSectionalStrategy):
        self.config = config
    ...
    def load_or_download_price_panel(self, tickers: Iterable[str]) -> pd.DataFrame:
        ...
        start = str(self.config.start_date or "full")
        end = str(self.config.end_date or "full")
        freq = str(self.config.rebalance_frequency)
    ...
    # ------------------------------------------------------------------
    # Cross-sectional snapshot / signal preparation
    # ------------------------------------------------------------------
    def build_raw_snapshot_for_date(self, signal_date: pd.Timestamp) -> pd.DataFrame:
    ...
    def build_signals_for_date(self, signal_date: pd.Timestamp) -> pd.DataFrame:
        raw_df = self.build_raw_snapshot_for_date(signal_date)
        if raw_df.empty:
            return raw_df

        enriched_df = self.strategy.enrich_raw_snapshot(self, raw_df.copy(), signal_date)
        filtered_df = self.apply_signal_filters(enriched_df)
        ...
        if self.strategy.score_column not in signals_df.columns:
            raise ValueError(
                f"Strategy {self.strategy.name!r} did not return score column "
                f"{self.strategy.score_column!r}."
            )

        sort_ascending = self.strategy.sort_ascending
        return (
    ...
    def apply_signal_filters(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        out = out[
            (out["adj_close_t"] > self.config.min_price)
            & (out["market_cap"] > self.config.min_market_cap)
            & (out["book_equity"] > self.config.min_book_equity)
        ].copy()

        if not self.config.allow_negative_earnings:
    ...
    def get_price_n_trading_days_before(
        self,
        ticker: str,
        date: pd.Timestamp,
        n_days: int,
        ...
        """
        Helper for future strategies such as modified FF / Carhart momentum.
        Returns the price n trading days before the given date.
        """
        if self._price_panel is None or ticker not in self._price_panel.columns:
            return np.nan

        valid_dates = self._price_panel.index[self._price_panel.index <= date]
        if len(valid_dates) <= n_days:
            return np.nan

        target_date = valid_dates[-(n_days + 1)]
        value = self._price_panel.at[target_date, ticker]
        return float(value) if pd.notna(value) else np.nan
    ...
    def universe_tickers_from_lifecycle(self) -> List[str]:
