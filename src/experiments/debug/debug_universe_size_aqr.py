...
def main() -> None:
    ...
    for date in calendar["signal_date"]:
        raw_df = base.build_raw_snapshot_for_date(date)

        if raw_df.empty:
            results.append((date, 0))
            continue

        enriched_df = strategy.enrich_raw_snapshot(base, raw_df.copy(), date)
        generic_filtered_df = base.apply_signal_filters(enriched_df)

        results.append((date, len(generic_filtered_df)))

    # convert to DataFrame for inspection
    df_counts = pd.DataFrame(results, columns=["date", "clean_universe_size"])

    print(df_counts.head())
    print(df_counts.describe())

    output_path = "data/processed/clean_universe_size.csv"
